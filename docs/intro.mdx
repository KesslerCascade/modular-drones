...
Modular Drones allows you to create custom personal drones or quadcopters. They are based on your design, and when active,
...
Rotor blocks have been added that allow the drone to actually fly. There's a basic wooden rotor, an advanced rotor, and a strong ion thruster.
...
<Callout variant="info">
    Bigger drones with more blocks will make the drone heavier and slower. Add more rotors or thrusters to compensate for this. Better thrusters provide more power.
</Callout>
...
Note that the position of the controller **must** be the same relative to the frame blocks as it was when the drone was originally constructed. The assembly frames face
a specific direction, indicated by the arrow, so if you move your drone assembly platform make sure to take note of which side/position the controller was on.
...
If the **Curios** (NeoForge), **Trinkets** (Fabric), or **Accessories** mod is installed, a dedicated slot to place the drone into is available. For backwards compatibility,
the cosmetic head slot can also be used, but only with the Accessories mod.
